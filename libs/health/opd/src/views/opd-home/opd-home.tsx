import { FhirService, NameField, ReferenceDisplayField, ResourcerefAutocomplete, ResourceTable, SimpleActionField, useFhirQuery  } from '@ha/appfhir';
 ...
 import LabReport from '../../components/lab-report/lab-report';
     ...
     <div className={styles['container']}>
         <form onSubmit={encounterForm.handleSubmit(onCreate)}>
           <Grid container justifyContent={'space-between'} alignItems={'center'} spacing={1}  alignContent={'center'} >
                 ...
                 <Button variant={'contained'} onClick={()=>setshowvital(true)}>vital</Button>
                 <Button variant={'contained'} onClick={()=>setshowpharmacy(true)}>pharmacy</Button>
                 <Button variant={'contained'} onClick={()=>setshowlab(true)} >Lab Report</Button>
         ...
         <Dialog fullWidth maxWidth={'lg'} open={showvital} onClose={()=>setshowvital(false)}>
             <DialogTitle display='flex' justifyContent={'space-between'}>

               <IconButton onClick={()=>setshowvital(false)} ><Icon>close</Icon></IconButton>
             </DialogTitle>
             <DialogContent>
             <Vital/>
             </DialogContent>
         </Dialog>
         <Dialog  open={showpharmacy} onClose={()=>setshowpharmacy(false)}>
             <Pharmacy/>

          </Dialog>
          <Dialog  open={showlab} onClose={()=>setshowlab(false)}>
           <LabReport/>
